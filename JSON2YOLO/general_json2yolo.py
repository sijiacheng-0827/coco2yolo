...
def convert_coco_json(json_dir='../coco/annotations/', use_segments=False, cls91to80=False):
    ...
    # save_dir = make_dirs(r'D:/data')  # output directory
    save_dir = r'C:\Users\PC\Desktop\all\yolo'  # output directory
    coco80 = coco91_to_coco80_class()

    # Import json
    for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
        fn = Path(save_dir) / 'labels' / json_file.stem.replace('instances_', '')  # folder name
        fn.mkdir()  # WindowsPath('D:/data/labels/coco')
        with open(json_file) as f:
            data = json.load(f)
            # has 5 keys: 'info', 'licenses', 'images', 'annotations', categories'
        # Create image dict
...
if __name__ == '__main__':
    ...
    if source == 'COCO':
        convert_coco_json(r'C:\Users\PC\Desktop\all',  # directory with *.json
                          use_segments=True,
                          cls91to80=True)
